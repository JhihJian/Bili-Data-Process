...
public class XmlLoader {
   ...
   */
  public static Chat loadChatFromXml(File file) throws InterruptedException {
    Chat chat =
        new Chat(
  ...
  private static String replaceChar(String str) {
    ...
    for (int i = 0; i < chars.length; i++) {
      if (chars[i] == 0x9
          || chars[i] == 0xA
          || chars[i] == 0xD
          || (chars[i] >= 0x20 && chars[i] <= 0xD7FF)
          || (chars[i] >= 0xE000 && chars[i] <= 0xFFFD)
          || (chars[i] >= 0x10000 && chars[i] <= 0x10FFFF)) {
        continue;
      }
      chars[i] = ' ';
    }
